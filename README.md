...
| | `zitadel_test_smtp_config` | Send a real test mail through a provider (active one by default) — the authoritative "does it deliver" check; reports the relay's rejection reason on failure |
...
- **SMTP tools are the one deliberate exception to the Management-API-only design.** `zitadel_get_smtp_config` / `zitadel_set_smtp_config` / `zitadel_activate_smtp_config` / `zitadel_test_smtp_config` use the **Admin API** (`/admin/v1/email/*`) because the notification SMTP provider is an **instance-level** resource shared by every org. ZITADEL gates it behind `iam.read` / `iam.write`, so the service account needs an **IAM-level manager** grant (e.g. `IAM_OWNER`) — `ORG_OWNER` alone returns `403`. Only grant this if you actually use the SMTP tools.
- **SMTP creds stay out of the conversation.** `zitadel_set_smtp_config` reads the relay creds from a gitignored file selected by a **non-secret `credsProfile`**, kept in a **per-provider folder** `~/.secrets/<provider>/.env[.<profile>]` (e.g. `~/.secrets/brevo/.env.accelerator`), holding `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM`. The server **auto-discovers** the one `~/.secrets/*/` folder whose `.env[.<profile>]` defines `SMTP_*`; pass **`credsDir`** (e.g. `"brevo"`) to disambiguate when several providers are set up in parallel, or `SMTP_ENV_PATH` to override the base path entirely. So you call the tool with just a profile name and the password never lands in the transcript. Passing `password` (or any field) as an argument still works and overrides the file — but the `password` argument *does* end up in the transcript, so prefer `credsProfile`. The password is redacted from logs and never returned by the read tool.
- **`zitadel_test_smtp_config` verifies delivery.** `[ACTIVE]` only means *selected* — a provider with a wrong SMTP key stays active but bounces every mail. The test tool sends a real message (by provider id, reusing the stored password, so no secret in the request) and surfaces the relay's own rejection reason on failure (e.g. `could not add smtp auth`). It's the one place `request()` opts into exposing the upstream error detail, because that reason describes the *relay* config, not ZITADEL internals.
