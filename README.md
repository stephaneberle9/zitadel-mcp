...
- **SMTP creds stay out of the conversation.** `zitadel_set_smtp_config` reads the relay creds from a gitignored file selected by a **non-secret `credsProfile`** — `~/.secrets/smtp/.env.<profile>` (default `~/.secrets/smtp/.env`; override the base with `SMTP_ENV_PATH`), holding `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM`. So you call the tool with just a profile name and the password never lands in the transcript. Passing `password` (or any field) as an argument still works and overrides the file — but the `password` argument *does* end up in the transcript, so prefer `credsProfile`. The password is redacted from logs and never returned by the read tool.
